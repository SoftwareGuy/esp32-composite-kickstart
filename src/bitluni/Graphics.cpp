...
Graphics::Graphics(int w, int h, int initialTrinagleBufferSize) :xres(w), yres(h) {
  font = 0;
...
void Graphics::print(const char *str)
{
	if(!font) return;
	while(*str) {
		if(*str >= 32 && *str < 128) font->drawChar(*this, cursorX, cursorY, *str, frontColor, backColor);
		cursorX += font->xres;
